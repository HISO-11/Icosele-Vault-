...
class MainWindow(QMainWindow):
    ...
    def _on_vm_selected(self, index: int) -> None:
        ...
        self.vm_controls.set_kvm_status(kvm_available())
    ...
    def _on_start(self) -> None:
        ...
        # RAM safety check: warn if VM requests more than 80% of available RAM
        try:
            for line in Path("/proc/meminfo").read_text().splitlines():
                if line.startswith("MemAvailable:"):
                    avail_kb = int(line.split()[1])
                    avail_mb = avail_kb // 1024
                    if cfg.ram_mb > avail_mb * 0.8:
                        recommended = max(512, (int(avail_mb * 0.5) // 512) * 512)
                        reply = QMessageBox.warning(
                            self, "Low Memory Warning",
                            f"This VM requests {cfg.ram_mb} MB but only "
                            f"{avail_mb} MB is available on your system.\n\n"
                            f"Starting may cause instability. Reduce RAM to "
                            f"{recommended} MB or lower for best results.\n\n"
                            f"Continue anyway?",
        ...
        self.vm_controls.set_kvm_status(proc._kvm_enabled)
