...
class SnapshotDAGPanel(QFrame):
    ...
    def _build_ui(self) -> None:
        ...
        # tag_filter → _dag is connected in _build_dag_tab after _dag is created

    # -- DAG tab — simple list for reliable selection --
    def _build_dag_tab(self) -> QWidget:
        ...
        self._snap_list.setStyleSheet(f"""
            QListWidget {{ background: {BG_DEEP}; border: none; color: {TEXT_PRIMARY};
                          font-size: 13px; font-family: {FONT_FAMILY}; }}
            QListWidget::item {{ padding: 10px 16px; border-bottom: 1px solid #313244; }}
            QListWidget::item:selected {{ background: #313244; }}
            QListWidget::item:hover {{ background: #2a2f2d; }}
        """)
        self._snap_list.currentItemChanged.connect(self._on_list_selection)
        lay.addWidget(self._snap_list, 1)

        # Also keep the DAG canvas for the sub-tab (but list is primary)
        self._dag = SnapshotDAGCanvas()
        # Don't add DAG to layout — it's available for other sub-tabs if needed
        return w
    ...
    def _on_list_selection(self, current, previous) -> None:
        ...
        log.info("Snapshot selected: %s (id=%s)", current.text(), snap_id)
    ...
    def set_enabled(self, enabled: bool) -> None:
        ...
        # Buttons always enabled — they work via qemu-img when VM is stopped
    ...
    def _refresh(self) -> None:
        ...
        # Also update DAG canvas if it exists
        try:
    ...
    def _on_create(self) -> None:
        log.info("[SNAPSHOT] Create button clicked — vm_id=%s, running=%s", self._vm_id, self._is_running)
        name, ok = QInputDialog.getText(self, "Create Snapshot", "Snapshot name:")
        if not ok or not name.strip():
            return
        name = name.strip()
        if self._is_running:
            log.info("[SNAPSHOT] Sending QMP savevm for '%s'", name)
            self.snapshot_action.emit("create", name)
        elif self._disk_path and Path(self._disk_path).exists():
            log.info("[SNAPSHOT] Running qemu-img snapshot -c '%s' '%s'", name, self._disk_path)
            try:
                ...
                QMessageBox.critical(self, "Error", "qemu-img not found. Install QEMU tools.")
                return
            except Exception as exc:
                QMessageBox.critical(self, "Error", f"Failed:\n{exc}")
                return
        else:
            QMessageBox.warning(self, "No Disk", "No disk image — cannot create snapshot.")
            return
        add_snapshot(self._vm_id, name,
                     parent_id=self._current_snap_id,
                     branch_name=self._current_branch)
        self._refresh()
        QMessageBox.information(self, "Success", f"Snapshot \"{name}\" created.")
        log.info("[SNAPSHOT] Created successfully: '%s'", name)

    def _on_restore(self) -> None:
        log.info("[SNAPSHOT] Restore button clicked — selected=%s", self._current_snap_id)
        if not self._current_snap_id:
            QMessageBox.information(self, "Restore", "Please select a snapshot from the list first.")
            return
        ...
        if QMessageBox.question(
                ...
                f"Restore to \"{s['name']}\"? Current state will be lost.",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        ) != QMessageBox.StandardButton.Yes:
            return
        if self._is_running:
            log.info("[SNAPSHOT] Sending QMP loadvm for '%s'", s["name"])
            self.snapshot_action.emit("restore", s["name"])
        elif self._disk_path and Path(self._disk_path).exists():
            log.info("[SNAPSHOT] Running qemu-img snapshot -a '%s' '%s'", s["name"], self._disk_path)
            try:
                result = subprocess.run(
                    ["qemu-img", "snapshot", "-a", s["name"], self._disk_path],
                    capture_output=True, timeout=30)
                if result.returncode != 0:
                    err = result.stderr.decode(errors="replace")[:300]
            ...
            QMessageBox.warning(self, "Error", "No disk image to restore.")
            return
        QMessageBox.information(self, "Restored", f"Restored to \"{s['name']}\".")
        log.info("[SNAPSHOT] Restored successfully: '%s'", s["name"])

    def _on_delete(self) -> None:
        log.info("[SNAPSHOT] Delete button clicked — selected=%s", self._current_snap_id)
        if not self._current_snap_id:
            QMessageBox.information(self, "Delete", "Please select a snapshot from the list first.")
            return
        s = get_snap_by_id(self._vm_id, self._current_snap_id)
        if not s:
            QMessageBox.information(self, "Delete", "Please select a snapshot from the list first.")
        ...
        if QMessageBox.question(
                ...
                f"Delete snapshot \"{s['name']}\"? This cannot be undone.",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        ) != QMessageBox.StandardButton.Yes:
            return
        if self._disk_path and Path(self._disk_path).exists():
            log.info("[SNAPSHOT] Running qemu-img snapshot -d '%s' '%s'", s["name"], self._disk_path)
            try:
                result = subprocess.run(
                    ["qemu-img", "snapshot", "-d", s["name"], self._disk_path],
                    capture_output=True, timeout=30)
                if result.returncode != 0:
                    err = result.stderr.decode(errors="replace")[:300]
        ...
        self._current_snap_id = None
        self._refresh()
        QMessageBox.information(self, "Deleted", f"Snapshot \"{s['name']}\" deleted.")
        log.info("[SNAPSHOT] Deleted successfully: '%s'", s["name"])
